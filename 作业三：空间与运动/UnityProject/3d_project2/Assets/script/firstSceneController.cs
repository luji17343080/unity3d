...
public class boatController{
	...
	public void move(){ // 船移动函数：从一端移动到另一端 
		if (size == "right") {
...
public class peopleController{ //角色控制类 
	...
	public void getOnBoat(boatController boatCtrl){ //上船，状态设置为“boat” 
		status = "boat";
	...
	public void getOffBoat(environmentController envCtrl){ //离开船（上岸），状态设置为“shore” 
		status = "shore";
		people.transform.parent = null;
		people.transform.position = envCtrl.getPosVec(size , number );
	}

	public void reset( environmentController envCtrl ){ //角色重置：状态设置为“shore”，位置设置为默认的岸上位置 
		status = "shore";
		size = defaultSize;
		people.transform.parent = null;
		people.transform.position = envCtrl.getPosVec(size , number );
